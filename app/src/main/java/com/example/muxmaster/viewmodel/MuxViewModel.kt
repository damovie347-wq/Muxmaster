...
class MuxViewModel(private val app: Application) : AndroidViewModel(app) {
    ...
    fun removeAudio(id: Int) { audioTracks.removeAll { it.id == id } }
    ...
    fun removeSubtitle(id: Int) { subtitleTracks.removeAll { it.id == id } }
    ...
    fun startMux() {
        ...
        if (video == null) { resultMessage = "Önce bir video dosyası seçin."; isSuccess = false; return }
            ...
            try {
                ...
                for ((i, track) in enabledAudio.withIndex()) {
                    if (track.source == TrackSource.NEW_FILE && track.fileUri != null) {
                        ...
                        if (cp == null) { resultMessage = "Ses dosyası okunamadı: ${track.fileDisplayName}"; return@launch }
                ...
                for (track in enabledSubs) {
                    if (track.source == TrackSource.NEW_FILE && track.fileUri != null) {
                        ...
                        if (cp == null) { resultMessage = "Altyazı okunamadı: ${track.fileDisplayName}"; return@launch }
                ...
                if (rc == null || rc != 0) { resultMessage = "FFmpeg hatası (kod: $rc)."; return@launch }
                if (!tempOutput.exists() || tempOutput.length() == 0L) { resultMessage = "Hata: Çıktı dosyası oluşmadı (0 byte)."; return@launch }
                ...
                if (copyOk) {
                    resultMessage = "✅ Tamamlandı! %.1f MB".format(finalBytes / (1024f*1024f))
                    isSuccess = true
                } else {
    ...
    private suspend fun resolveAudioPlan(track: AudioTrackItem, videoPath: String, workDir: File, runTempFiles: MutableList<File>): MapPlan = withContext(Dispatchers.IO) {
        if (track.source == TrackSource.EXISTING) {
            ...
            if (!ReturnCode.isSuccess(s.returnCode) || !extracted.exists() || extracted.length()==0L) return@withContext MapPlan.Failed("Ses track ayıklanamadı (#${track.existingStreamIndex})")
    ...
    private suspend fun resolveSubtitlePlan(track: SubtitleTrackItem, videoPath: String, workDir: File, runTempFiles: MutableList<File>): MapPlan = withContext(Dispatchers.IO) {
        if (track.source == TrackSource.EXISTING) {
            ...
            val s = FFmpegKit.executeWithArguments(arrayOf("-y","-i",videoPath,"-map","0:${track.existingStreamIndex}","-c","copy",extracted.absolutePath))
            if (!ReturnCode.isSuccess(s.returnCode) || !extracted.exists() || extracted.length()==0L) return@withContext MapPlan.Failed("Altyazı ayıklanamadı (#${track.existingStreamIndex})")
            runTempFiles.add(extracted); MapPlan.Separate(extracted.absolutePath, track.delayMs)
        } else {
    ...
    private fun buildFfmpegArgs(videoPath: String, audioPlans: List<MapPlan>, audioTracks: List<AudioTrackItem>, subPlans: List<MapPlan>, subTracks: List<SubtitleTrackItem>, outputPath: String): List<String> {
        ...
        audioTracks.forEachIndexed { i, t -> if (t.title.isNotBlank()) args += listOf("-metadata:s:a:$i","title=${t.title}"); if (t.language.isNotBlank() && t.language!="und") args += listOf("-metadata:s:a:$i","language=${t.language}"); args += listOf("-disposition:a:$i",if(t.isDefault)"default" else "0") }
